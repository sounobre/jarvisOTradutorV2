...
def create_list(infile):
    ...
    with open(infile + '.used', 'w', encoding='utf-8') as outfile:
        in_lists = []
        in_list = []
        ctr = 0
...
def open_emb_file(file_name):
    emb_dict = {}
    emb_file = open(file_name + '.used', 'r', encoding='utf-8')
...
def write_path_to_file(path, file_name, total_score):
    ...
    for i in pathlist:
        ...
        ctr += 1
    with open(output_folder + '/' + file_name + '.path', 'w', encoding='utf-8') as fo:
        fo.write(out_path)
...
def write_pairs_to_file(pairs, file_name, total_score):
    ...
    with open(output_folder + '/' + file_name + '.aligned', 'w', encoding='utf-8') as fo:
        ctr = 0
        for p in pairs:
...
if __name__ == '__main__':
    ...
    files2align = {}
    filesdone = {}
    with open(corpus_folder + '/files2align.txt', 'r', encoding='utf-8') as fa, open(
                corpus_folder + '/filesdone.txt', 'r', encoding='utf-8') as fd:
        for line in fd:
    ...
    try:
        while len(alignlist) > 0:
            ...
            if len(alignlist) > 0:
                ...
                filesdonefile = open(corpus_folder + '/filesdone.txt', 'a', encoding='utf-8')
